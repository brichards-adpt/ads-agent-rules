...
## **6\. Changelog & Improvement Tracking**

Every improvement made to the product MUST be documented to maintain a comprehensive, team-readable history of changes.

### **6.1. When to Log**

* Log an entry whenever you complete a discrete improvement — a bug fix, feature addition, refactor, token correction, or accessibility improvement.
* Log **before** committing, so the changelog entry and the commit are always in sync.
...
**Type:** Fix | Feature | Refactor | Accessibility | Style | Chore
**Summary:** One-sentence description of what changed and why.
**Files affected:** List of modified files.
**Commit ref:** <short commit hash once committed>
```

### **6.3. Changelog Rules**

* **Branch is mandatory:** Every entry MUST include the exact Git branch name at the time of the change (retrieve via `git branch --show-current`). Never omit or approximate it.
* **Timestamp is mandatory:** Use ISO 8601 format (YYYY-MM-DD HH:MM) in local time.
* **Append only:** New entries are always added at the **top** of the changelog (newest first) so the most recent changes are immediately visible.
* **File location:** Maintain the log in `CHANGELOG.md` at the project root. Create it if it does not exist.
* **No vague summaries:** Summaries must be specific enough for a team member to understand the change without reading the code (e.g., "Replaced hardcoded #FF0000 with color.text.danger token" not "Fixed colour").
